...
interface VacancyRepository : JpaRepository<Vacancy, UUID> {
    ...
    fun findByUserIdAndUrl(userId: UUID, url: String): Optional<Vacancy>

    fun findByIdAndUserId(id: UUID, userId: UUID): Optional<Vacancy>

    fun findByUserIdOrderByCreatedAtDesc(userId: UUID): List<Vacancy>

    fun findByUserIdAndStatusOrderByCreatedAtDesc(userId: UUID, status: VacancyStatus): List<Vacancy>
    ...
    fun findArchivedByUserId(@Param("userId") userId: UUID): List<Vacancy>

    @Modifying
    @Query("UPDATE Vacancy v SET v.deleted = true, v.updatedAt = CURRENT_TIMESTAMP WHERE v.id = :id AND v.user.id = :userId")
    fun softDeleteByIdAndUserId(@Param("id") id: UUID, @Param("userId") userId: UUID): Int
