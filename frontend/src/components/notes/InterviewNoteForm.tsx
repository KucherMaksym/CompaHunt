...
interface InterviewNoteFormData {
  content: string
  type: NoteType
...
export function InterviewNoteForm({ interviewId, isVisible, onCancel, onSuccess }: InterviewNoteFormProps) {
  const queryClient = useQueryClient()
  
  const [formData, setFormData] = useState<InterviewNoteFormData>({
    content: '',
  ...
  const createNoteMutation = useMutation({
    mutationFn: async (data: InterviewNoteFormData) => {
      return toast.promise(notesApi.createInterviewNote({
            interviewId,
            content: data.content,
            type: data.type,
            priority: data.priority
          }), {
            loading: 'Creating interview note...',
            success: 'Interview note added successfully!',
            error: 'An error occurred while creating interview note.',
          }
      )
    },
    onSuccess: () => {
  ...
  const handleChange = (field: keyof InterviewNoteFormData, value: string) => {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <Textarea
              ...
              maxLength={300}
              rows={4}
              placeholder="Add your interview note here..."
