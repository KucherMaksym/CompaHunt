...
import { NotePriority, NoteType } from '@/types/vacancy'
import { InterviewNote, notesApi } from '@/lib/api/notes'
...
const getTypeColor = (type: NoteType) => {
  switch (type) {
    ...
    case NoteType.RESEARCH: return 'text-green-600 border-green-600 bg-green-600/10'
    case NoteType.NEXT_STEPS: return 'text-indigo-600 border-indigo-600 bg-indigo-600/10'
    case NoteType.CONTACT_INFO: return 'text-orange-600 border-orange-600 bg-orange-600/10'
    case NoteType.COMPANY_CULTURE: return 'text-pink-600 border-pink-600 bg-pink-600/10'
    default: return 'text-gray-600 border-gray-600 bg-gray-600/10'
  }
}

interface InterviewNoteListProps {
  notes: InterviewNote[]
  interviewId: string
  onAddNew: () => void
}

export function InterviewNoteList({ notes, interviewId, onAddNew }: InterviewNoteListProps) {
  ...
  const deleteNoteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['interviewNotes', interviewId] })
    }
  })

  const onDelete = async (noteId: string) => {
    if (confirm('Are you sure you want to delete this interview note?')) {
      try {
      ...
      <Card className="text-center py-12">
        <CardContent>
          ...
          <p className="text-muted-foreground mb-4">
            Add your first interview note to get started
          </p>
          <Button 
            onClick={onAddNew}
            variant="outline"
